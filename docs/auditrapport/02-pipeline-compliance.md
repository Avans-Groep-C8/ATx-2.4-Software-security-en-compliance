...
|---|---|---|---|
| **8.4 / 8.32** Toegang broncode & wijzigingsbeheer | Toegang tot broncode is beperkt; elke wijziging is traceerbaar naar een persoon. | Trunk-based op `main` met branch protection/ruleset: alleen via PR, minimaal 1 review, alle CI-checks moeten slagen, force-push en directe deletes geblokkeerd (ook voor admins). MFA verplicht voor alle GitHub-accounts; RBAC op de repo; GitHub Audit Log. | Signed commits (PGP) nog niet ingeschakeld in ruleset. Ruleset en environments vastgelegd in [00-auditrapport.md](00-auditrapport.md) §6.2 + `evidence/`. |
...
| **8.9** Configuratiebeheer | Configuraties beheerd, gedocumenteerd en beschermd tegen ongeautoriseerde wijziging. | Pipeline-as-code: workflows in `.github/workflows/`, versiebeheerd en reviewable. Quality gates blokkeren bij falen. Secrets via GitHub (Environment) Secrets, nooit hardcoded. Immutable artifacts (een fix is een nieuwe build). | Alerting op wijzigingen in workflow-bestanden nog in te richten (zie 8.16). |
| **8.16** Monitoring van de pipeline | Activiteiten worden gelogd en gecontroleerd op afwijkingen. | GitHub Security tab + Dependabot alerts; GitHub Audit Log; webhooks/notificaties bij kritieke events (mislukte runs, secret-gebruik, wijziging van workflows, deployment-events). | Alerting op workflow-wijzigingen nog in te richten; grotendeels op organisatieniveau te beleggen. |
...
| **8.28** Veilig programmeren | Vastgestelde richtlijnen voor veilig coderen, die worden gecontroleerd. | Coding standards (OWASP Secure Coding Practices); SAST via CodeQL en Snyk Code; GitHub Secret Scanning + pre-commit hooks (detect-secrets); dependency pinning (exacte versies/hashes, geen `latest`). | Secret scanning en pre-commit hooks nog te activeren in de CI. |
...
| **8.31** Scheiding van omgevingen (OTAP) | Gescheiden ontwikkel-, test-, acceptatie- en productieomgeving; productiedata nooit naar een lagere omgeving; gescheiden secrets. | GitHub Environments (`develop`, `test`, `production`) met protection rules; gescheiden secrets per omgeving; productie achter goedkeuring via `promote-prod.yml`; aparte `docker-compose`-bestanden per omgeving. | Pentest op lokale RefApp; productie-hosting ziekenhuis buiten modulescope. |
...
- [x] Branch protection / ruleset actief op `main` — PR, reviewer, status checks, geen force-push
- [x] CI-checks verplicht vóór merge (`build and test`, CodeQL)
- [x] CodeQL actief
- [ ] Secret Scanning actief (org-niveau)
- [ ] Dependabot alerts + security updates actief
- [ ] Dependency Review Action gekoppeld aan PR's
- [x] SBOM gegenereerd (CycloneDX) en geanalyseerd (SCA via Snyk)
- [x] GitHub Environments gedefinieerd (`develop`, `test`, `production`)
- [x] Secrets gescheiden per environment (waar van toepassing)
- [x] Pipeline-artifacts (rapporten, SBOM) worden bewaard
- [ ] README.md beschrijft beleid en procedure (mini-ISMS)

## 4. Conclusie

Dit plan legt vast hoe de CI/CD-pipeline vanaf het ontwerp aan NEN-7510:2024-2 gaat voldoen. **Ingevoerd:** trunk-based PR-flow met ruleset op `main`, GitHub Environments OTAP, SAST (CodeQL/Snyk), SCA, SBOM en artifact-retentie — bewijs in [00-auditrapport.md](00-auditrapport.md) §6.2 en `evidence/`. **Restrisico's:** formeel patchbeleid documenteren, Dependabot/Dependency Review, Snyk als hard gate, signed commits optioneel.
